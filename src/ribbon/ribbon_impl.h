...
#include "ribbon_fastrange.h"
#include "ribbon_port.h"  // for PREFETCH
...
class SerializableInterleavedSolution {
 public:
  ...
  //用于在反序列化时重置数据
  void ResetSerializableInterleavedSolution(char* data, size_t data_len){
    data_ = data;
    data_len_ = data_len;
  }
  void PrepareForNumStarts(Index num_starts) {
  ...
  ResultRow PhsfQuery(const Key& input, const PhsfQueryHasher& hasher) const {
    ...
    if (TS::kAllowZeroStarts && num_starts_ == 0) {
      ...
      Index num_columns;
      Index start_bit;
      // 为 InterleavedSolutionStorage 中的键预取内存。
  ...
  const uint32_t* hash_values = hasher.GetHashValues(input);
  for (size_t i = 0; i < kNumHashes; i++) {
    if (!filter_data_[hash_values[i] % filter_size_]) {
      return false;
    }
  }
  return true;
